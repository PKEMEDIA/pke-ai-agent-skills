---
name: generate2dmap
description: Generate production-oriented 2D game maps with image_gen - RPG/top-down maps, side-scroller parallax stages, tilemaps, layered raster maps, prop packs, collision zones, and walkable areas. Use when building browser games that need real map art (not pure code-drawn tiles), layered props, or map collision metadata. Triggers on "map", "level", "stage", "tilemap", "overworld", "dungeon", "side scroller background", "prop pack", "2D map".
metadata:
...
| Image tools | `image_gen` / `image_edit` (see **`imagine`**) |
| Inspect images | `read_file` on PNG paths (not Codex view_image) |
| Generated image path | Path returned by `image_gen` / `image_edit` — copy into `assets/map/` |
...
2. Immediately before the next `image_gen` call, make that exact image visible in conversation context. If it is a local file, call `read_file` on the saved file (image understanding).
...
2. Make the base image visible in conversation context. If the base is a local file, use `read_file` immediately before calling built-in `image_gen`; do not rely on a path string as the reference.
...
2. Make the background visible in conversation context. If it is a local file, use `read_file` immediately before calling built-in `image_gen`; do not rely on a path string as the reference.
...
When building PKE / Pretty Kitty Media brand surfaces, also load `pke-face-lock`, `pke-official-black-mask`, and `skill-orchestrator`. Autonomous free-tier improve - `pke-synthetic-intellect` (`bash /workspace/scripts/pke-learn.sh`). Do not invent a second company face or recolor the official mask.
