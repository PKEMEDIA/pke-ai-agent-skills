# PKE Full Ecosystem Map

## Surfaces

| Surface | Integration |
|---|---|
| Grok iOS | Skill triggers from chat; attach face/mask refs |
| Grok web | Same skills + Brand Guidelines preview |
| Grok Imagine | User-driven only; mild base → edit; locks from face/mask skills |
| Grok Build | Brand app + `public/pke/` + scripts |
| GitHub | `PKEMEDIA/pke-ai-agent-skills` |
| Free tier | `pke-learn.sh` / `pke-self-heal.sh` — zero Imagine |
| Local ComfyUI | `comfyui/pke-face-lock-base.json` FaceID + OpenPose |

## Agents (skills)

| Skill | Class | Role |
|---|---|---|
| pke-face-lock | Brand lock | Company face |
| pke-official-black-mask | Brand lock | Official mask |
| skill-orchestrator | Meta | Health, heal, export |
| pke-synthetic-intellect | Meta | Autonomous learn |
| skill-creator | Meta | Validate / scaffold |
| design-ui / imagine / … | App Builder | Product surfaces; ecosystem hook footer |

## Deploy order

1. Validate all skills (`validate-skill.sh`)
2. Self-heal
3. Learn cycle
4. Brand app smoke
5. GitHub push brand pack + mind + scripts + ecosystem map

## Commands

```bash
bash /workspace/scripts/pke-self-heal.sh
bash /workspace/scripts/pke-learn.sh
bash /workspace/scripts/pke-learn.sh --push
```
